\section{Dependence of $\rho_{max}$ and $n$ on Eigenvalue}
...
This is, however, not a realistic possibility, and this sections is therefore dedicated to find and discuss on the optimal choices for $\rho_{max}$ and $n$ to obtain acceptable values for the eigenvalues of \matref{eq:NatureOfTheProblem5} with the matrix $\v{A}$ defined in \matref{eq:NatureOfTheProblem6}.
...
In the figure below, the dependence of different integer valued $\rho_{max}$ on the three first eigenvalues gained by computed Jacobi method is plotted for $n=100$. 
\begin{figure}[H]
	\centering
...
With this $\rho_{max}$, we wish to find the integer value of $n$ that gives the first three eigenvalues with four leading digits. 
This optimal $n$ is found by steady increment of $n$, as seen in \figref{fig:DependenceOnEigenvalue2}.
\begin{figure}[H]
	\centering
